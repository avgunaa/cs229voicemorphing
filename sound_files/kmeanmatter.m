...
for i = 1:49
    [sa_temp,f] = wavread(strcat('words/aaaw_dtw_c', int2str(i), '.wav'));
    sa = [sa; sa_temp];
    size(sa_temp);
    sg_temp = wavread(strcat('words/gggw_dtw_c', int2str(i),'.wav'));
    sg = [sg; sg_temp];
    size(sg_temp);
end
...
sg_new = wavread(strcat('words/gggw_dtw_c', int2str(1), '.wav'));
% sg_new1 = wavread(strcat('words/ggw_dtw_c', int2str(6), '.wav'));
%sg_new = [sg_new];
...
for i=1:num_Xg_new
    ...
    if i ~= 1
        tempXg = [Xg_new(i-1,end-kg+2:end)' ; Xg_new(i,:)' ];
        tempXa = [Xa_new(end-ka+2:end)'; zeros(p,1)];
    end
